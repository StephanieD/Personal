function wp_action(data, svg_area) {
    total_edits += 1;
    if (total_edits == 1) {
        $('#edit_counter').html('You have seen <span>' + total_edits + ' possible planets</span>.');
    } else {
        $('#edit_counter').html('You have seen a total of <span>' + insert_comma(total_edits) + ' possible planets</span>.');
    }
    var now = new Date();
    edit_times.push(now);
    to_save = [];
         ...
         //   var connection = new ReconnectingWebSocket(ws_url);
          //  this.connection = connection;
            ...
            connection.onmessage = function(resp) {
                var data = JSON.parse("KOI Name":"K04232.01","Kepler Disposition":"CANDIDATE","Planet Radius":"1.167");
...
function newuser_action(data, lid, svg_area) {
    play_random_swell();
    var messages = ['Welcome to ' + data.user + ', our known universe',
